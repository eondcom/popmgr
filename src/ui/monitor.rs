...
impl MonitorState {
    ...
    pub fn view(&self) -> Element<'_, MonitorMsg> {
        ...
        let mut col = column![text("하드웨어 모니터").size(20), Space::with_height(6), text("2초 간격 센서 상태 및 과열 보호").size(11).color(C_DIM), Space::with_height(14)].spacing(0);
        if let Some(r) = &self.running { col = col.push(running_bar(r)).push(Space::with_height(10)); }
        col = col.push(card(column![text("CPU").size(14), text(val(s.and_then(|x| x.package_temp), "°C")).size(28).color(temp_color(s.and_then(|x| x.package_temp))), text(format!("코어 최대 {} · 사용률 {} · 클럭 {}", val(s.and_then(|x| x.core_max_temp), "°C"), val(s.and_then(|x| x.cpu_usage), "%"), val(s.and_then(|x| x.clock_mhz), " MHz"))).size(11).color(C_DIM), text(format!("터보 {} · 스로틀 {} ({}) · 팬 {} · 프로파일 {}", s.and_then(|x| x.turbo_disabled).map(|b| if b { "꺼짐" } else { "켜짐" }).unwrap_or("지원 안 함"), s.and_then(|x| x.throttle_count).map(|v| v.to_string()).unwrap_or_else(|| "—".into()), val(s.and_then(|x| x.throttle_per_sec), "/초"), s.and_then(|x| x.fan_rpm).map(|v| format!("{v} RPM")).unwrap_or_else(|| "—".into()), s.and_then(|x| x.profile.as_deref()).unwrap_or("—"))).size(11).color(C_DIM)]));
        col = col.push(Space::with_height(10)).push(card(column![text("디스크").size(14), text(format!("NVMe {} · 사용률 {} · IO 정지 avg10 {}", val(s.and_then(|x| x.nvme_temp), "°C"), val(s.and_then(|x| x.disk_usage), "%"), val(s.and_then(|x| x.io_psi), ""))).size(12).color(C_TEXT)]));
        col = col.push(Space::with_height(10)).push(card(column![text("메모리").size(14), text(format!("{} / {} ({}) · 스왑 {} · TMEM {} · PSI {}", val(s.and_then(|x| x.mem_used_gib), " GiB"), val(s.and_then(|x| x.mem_total_gib), " GiB"), val(s.and_then(|x| x.mem_percent), "%"), val(s.and_then(|x| x.swap_used_gib), " GiB"), val(s.and_then(|x| x.tmem_temp), "°C"), val(s.and_then(|x| x.mem_psi), ""))).size(12).color(C_TEXT)]));
        ...
        col = col.push(Space::with_height(10)).push(card(column![text("GPU").size(14), text(gpu).size(12).color(C_TEXT)]));
        ...
        let helper_extra: Element<'_, MonitorMsg> = if !self.helper_installed {
            column![Space::with_height(8), text("권한 설정이 필요합니다.").size(11).color(C_DIM), action_btn("권한 설정 (pkexec, 1회)", MonitorMsg::InstallHelper, enabled, C_BLUE)].into()
        } else { Space::with_height(0).into() };
        col = col.push(Space::with_height(10)).push(card(column![text("과열 보호").size(14), text("90°C 이상이면 터보 차단, 78°C 이하 60초 유지 시 복원").size(11).color(C_DIM), text(format!("현재: {} · {}", s.and_then(|x| x.turbo_disabled).map(|b| if b { "터보 꺼짐" } else { "터보 켜짐" }).unwrap_or("지원 안 함"), if self.saved.turbo_off_by_popmgr { "popmgr가 차단함" } else { "사용자 상태 유지" })).size(11).color(C_TEXT), self.last_action.as_deref().map(|x| text(x).size(11).color(C_DIM)).unwrap_or_else(|| text("").size(1)), Space::with_height(8), row![action_btn(protect, MonitorMsg::ToggleProtect, enabled, C_BLUE), Space::with_width(8), action_btn("터보 끄기 지금", MonitorMsg::TurnOffNow, enabled && self.helper_installed, C_WARN), Space::with_width(8), action_btn("터보 켜기", MonitorMsg::TurnOnNow, enabled && self.helper_installed, C_OK),].spacing(0), helper_extra]));
