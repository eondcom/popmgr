...
fn gtk_module_bug_card(status: Option<&GtkModuleBugStatus>) -> Element<'_, ImeMsg> {
    let Some(status) = status else {
        return card(text("[i] GTK3 fcitx5 모듈 없음 — 진단 생략").size(13).color(C_DIM));
    };
    ...
    let mut body = column![text(gtk_bug_title(status.state)).size(13).color(color)].spacing(6);
    if status.state != GtkModuleBugState::Ok {
        body = body.push(text(gtk_bug_description(status.state)).size(11).color(C_DIM));
        for (package, version) in &status.packages {
            body = body.push(text(format!(
                "{package}: {}", version.as_deref().unwrap_or("미설치")
            )).size(11).color(C_TEXT));
        }
        body = body.push(text("https://bugs.kali.org/view.php?id=9146").size(11).color(C_BLUE));
        if let Some(command) = &status.fix_command {
            body = body.push(text("안내만 제공합니다. 아래 텍스트를 선택해 복사할 수 있습니다.")
                .size(11).color(C_DIM));
            // 입력 변경은 무시하되 선택 및 키보드 복사는 허용한다. 실행 버튼은 없다.
            body = body.push(iced::widget::text_input("", command).on_input(|_| ImeMsg::Noop).size(11));
        }
    }
    container(body)
        .width(Length::Fill)
        ...
        .style(move |_| iced::widget::container::Style {
            ...
            border: iced::Border { radius: 8.0.into(), color, width: 1.0 },
            ..Default::default()
        })
        .into()
}
